...
GENERATED_DIR=nora_fleet/api/grpc
...
#   from both nora-fleet and the external project are mixed, this sometimes
#   causes errors in the form of one of 2 errors:
#
#       1)  TypeError: Couldn't build proto file into descriptor pool:
#               Depends on file 'nora_fleet/grpc/generated/agent.proto',
#               but it has not been loaded
#       2)  TypeError: Couldn't build proto file into descriptor pool:
#               duplicate symbol 'nora_fleet.grpc.generated.agent.AgentStatus'
#
...
"${TOP_LEVEL}"/nora_fleet/api/scripts/check_openapi_plugin.sh
...
"${TOP_LEVEL}"/nora_fleet/api/scripts/convert_yaml_to_json.sh "${OPENAPI_RESULT}" "${REQUIRED_RESULT}"
