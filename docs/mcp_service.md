# Nora Fleet MCP service

Nora Fleet server runs MCP (Model Context Protocol) service.
...
in this case the server will provide both MCP and native nora-fleet API simultaneously.

Another option is to specify:

--mcp_only=true in the server command line or set environment variable AGENT_MCP_ONLY=true;
in this case the server will only provide MCP API endpoint, native nora-fleet API will be disabled.

## MCP service version

Nora Fleet implements MCP protocol version 2025-06-18
...
easy scalability of nora-fleet/MCP deployment.

## Agent networks as MCP tools

In the scope of MCP protocol, each public nora-fleet agent network is represented by an MCP tool
...
replicated from nora-fleet OpenAPI specification:
...
and tool description is what is returned by "function" nora-fleet API call.
...
Server response (example body):
    ...
    ```json
    {
      ...
      "result": {
        ...
        "serverInfo": { "name":"NoraFleet-MCPServer","title":"Nora Fleet MCP Server","version":"1.0.0" },
        "instructions": ""
      }
    }
    ```
