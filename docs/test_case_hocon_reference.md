...
This document describes the nora-fleet specifications for the test case .hocon files
...
| value            | meaning                                                                          |
|:-----------------|:---------------------------------------------------------------------------------|
| direct (default) | Connect directly to the agent via a nora-fleet library call - no server required. |
...
When using the [Assessor](../nora_fleet/test/assessor/assessor.py) tool to categorize modes of failure,
...
The [Assessor](../nora_fleet/test/assesor/assessor.py) is a tool which uses these data-driven test cases
