...
# Vendored third-party files that legitimately carry another party's
# copyright and license. These are NOT ours to relicense or scrub -- Google
# publishes them under Apache-2.0 and that notice must be preserved. They are
# exempt from the scan so the guard does not force a license violation.
VENDORED_EXEMPT = {
    "nora_fleet/api/grpc/google/api/annotations.proto",
    "nora_fleet/api/grpc/google/api/http.proto",
}
...
def is_exempt(path):
    ...
    return path in SELF_EXEMPT or path in VENDORED_EXEMPT or (
