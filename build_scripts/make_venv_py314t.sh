...
# Create a local virtual environment for running/developing the nora-fleet
...
#   * All dependencies -- including nora-common -- are
...
#     set it.
#
# nora-fleet itself is NOT installed into the venv; run it from the repo source
...
function main() {
   ...
   interpreter such as 3.14t, and that no stray override is in effect."
    ...
    # 2. Install dependencies straight from requirements.txt (nora-common
...
import nora_common
print("  nora_common         :", nora_common.__name__, "OK")
import nora_fleet
print("  nora_fleet import    : OK (from repo source)")
PY
...
To use it to run the nora-fleet service from source:

    source "${VENV_DIR}/bin/activate"
    export PYTHONPATH="${REPO_ROOT}"
    export AGENT_MANIFEST_FILE="${REPO_ROOT}/nora_fleet/registries/manifest.hocon"
    export AGENT_TOOL_PATH="${REPO_ROOT}/nora_fleet/coded_tools"
